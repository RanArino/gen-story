...
- [ ] Milestone 1 — Negative-prompt vocabulary in `packages/shared` (`BASE_NEGATIVE_PROMPT`, `RECOMMENDED_NEGATIVE_FENCE`, `composeNegativePrompt`), re-exported from `index.ts`, with unit tests.
- [ ] Milestone 2 — Domain + DTO + schema: add `negativePrompt: string` to the `Storyboard` and `Scene` models, the `StoryboardDto`/`SceneDto`, and the Zod input schemas.
- [ ] Milestone 3 — DB migration `0009_*` adds `storyboards.negative_prompt` and `scenes.negative_prompt` (both `text NOT NULL DEFAULT ''`); repository read/write updated.
- [ ] Milestone 4 — Composer: extend `composeImagePrompt` to append `, avoid: <merged negatives>`; thread the merged negative through `local-image-preprocessing.ts`; extend `prompt-composer.test.ts`.
- [ ] Milestone 5 — Preview endpoint `POST /api/scenes/:sceneId/preview-prompt` returning `{ prompt, negativePrompt }` from the same composition path, accepting optional override fields for unsaved edits. Route tests.
- [ ] Milestone 6 — Web UI: negative-prompt textareas (project + scene), "Insert recommended fence" action, and the collapsible composed-prompt preview panel in the scene editor and the ReviewPage regen modal; EN/JA i18n strings.
- [ ] Milestone 7 — End-to-end validation (`pnpm dev`, one real OpenAI call); typecheck + lint + tests green; screenshots captured.
- [ ] Flip the two §18.A rows from `🟡` to `✅` when Milestone 7 passes; update the summary table accordingly.
