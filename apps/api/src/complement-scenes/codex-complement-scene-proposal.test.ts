...
import { CodexComplementSceneProposalAdapter } from "./codex-complement-scene-proposal";
...
describe("CodexComplementSceneProposalAdapter", () => {
  it("maps validated Codex JSON into complement scene proposals", async () => {
    const adapter = new CodexComplementSceneProposalAdapter(
      new MemoryObjectStorage(onePixelPng),
      async (_request, parse) => parse(validProposalsJson, "codex-test"),
    );
  ...
  it("only sends non-deleted project photos as images", async () => {
    ...
    const adapter = new CodexComplementSceneProposalAdapter(
      ...
      async (request, parse) => {
        ...
        return parse(validProposalsJson, "codex-test");
      },
    );
