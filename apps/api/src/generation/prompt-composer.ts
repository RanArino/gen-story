const CAMERA_DESCRIPTORS: Record<string, string> = {
  ...
  "Extreme Wide":
    "extreme wide shot, tiny human presence, monumental environment scale",
  Medium: "medium shot, human eye level camera, natural perspective",
  "Close-up":
    "close-up shot, shallow depth of field, cinematic bokeh background, subject fills frame",
  "Extreme Close-up":
    "extreme close-up, single detail fills frame, abstract bokeh surrounds",
  Aerial: "overhead top-down shot, bird's eye view, abstract ground patterns",
  Overhead:
    "overhead top-down composition, abstract human patterns, cinematic drone perspective",
  POV: "first-person POV shot, immersive camera angle, subjective framing",
  "Low Angle":
    "dramatic low angle shot, three-point perspective, strong vertical convergence, horizon below frame",
  Telephoto:
    "telephoto compressed shot, long lens compression, shallow depth, cinematic isolation",
  Voyeur:
    "voyeur shot through crowd, foreground silhouettes heavily blurred, surveillance feeling",
};
...
const LIGHTING_DESCRIPTORS: Record<string, string> = {
  ...
  Night:
    "night atmosphere, urban ambient glow, neon rim lighting, deep shadows",
  ...
  Silhouette:
    "pure silhouette backlight, character readable outline, glowing haze",
  Volumetric:
    "volumetric light shafts, atmospheric particles, cinematic god rays",
};
...
export function composeImagePrompt(input: {
  ...
  segments.push(
    EMOTION_DESCRIPTORS[emotion] ?? "emotional cinematic atmosphere",
  );
  segments.push(
