...
import { ClaudeSceneFillGenerationAdapter } from "./claude-scene-fill-generation";
...
describe("ClaudeSceneFillGenerationAdapter", () => {
  it("maps validated Claude JSON into a scene fill suggestion", async () => {
    const adapter = new ClaudeSceneFillGenerationAdapter(
      new MemoryObjectStorage(onePixelPng),
      async (_request, parse) => parse(validSceneFillJson, "claude-test"),
    );
  ...
  it("sends the primary photo as an image and grounds the prompt in it", async () => {
    ...
    const adapter = new ClaudeSceneFillGenerationAdapter(
      ...
      async (request, parse) => {
        ...
        return parse(validSceneFillJson, "claude-test");
      },
    );
