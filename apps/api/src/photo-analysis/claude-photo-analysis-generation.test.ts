...
import { ClaudePhotoAnalysisGenerationAdapter } from "./claude-photo-analysis-generation";
...
describe("ClaudePhotoAnalysisGenerationAdapter", () => {
  it("maps validated Claude JSON into the application port result", async () => {
    const adapter = new ClaudePhotoAnalysisGenerationAdapter(
      new MemoryObjectStorage(onePixelPng),
      async (_request, parse) => parse(validPhotoAnalysisJson, "claude-test"),
    );
  ...
  it("sends one normalized image per photo and the built prompt", async () => {
    ...
    const adapter = new ClaudePhotoAnalysisGenerationAdapter(
      ...
      async (request, parse) => {
        ...
        return parse(validPhotoAnalysisJson, "claude-test");
      },
    );
  ...
  it("rejects malformed Claude JSON", async () => {
    const adapter = new ClaudePhotoAnalysisGenerationAdapter(
      ...
      async (_request, parse) =>
        parse(JSON.stringify({ emotionCandidates: [] }), "claude-test"),
    );
